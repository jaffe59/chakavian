...
#process verbal Cakavian endings from standard Croatian data
...
#input conllu data line by line - change matching entries to their Cakavian inflections
...
def inflect_genplural(line):
    ...
    if full_form in full_form_exceptions:
        #features += "|Dialect=Sto" mark this later during data split
        #fields[5] = features
        return " ".join(fields)
    else:
...
def inflect_dli_plural(line):
    ...
    # replace -ma with -n (for 'a' nouns)
        ...
        #features += "|Dialect=Sto" this will be done later during data split
    fields[1] = full_form
    fields[5] = features
    return " ".join(fields)
